...
public Bitmap XyPadForeground;
public Bitmap XyPadBackground;
...
private void Init() {
  ...
  XyPadForeground = null;
  XyPadBackground = null;
  ButtonOn = null;
  ButtonOff = null;
...
public bool Load(string name, Skin def) {
  Init();
  ...
  foreach(XmlNode n in x.ChildNodes[0].ChildNodes) {
    ...
    if(n.NodeType == XmlNodeType.Element) {
      ...
      case "XyPad":
        if(n.Attributes["foreground"] != null) {
          XyPadForeground = new Bitmap(AppPath + "\\Skins\\" + n.Attributes["foreground"].Value);
        }
        if(n.Attributes["background"] != null) {
          XyPadBackground = new Bitmap(AppPath + "\\Skins\\" + n.Attributes["background"].Value);
        }
        break;
      case "Button":
  ...
  if(def != null) {
    ...
    if(FaderForeground == null) FaderForeground = def.FaderForeground;
    if(FaderBackground == null) FaderBackground = def.FaderBackground;
    if(XyPadForeground == null) XyPadForeground = def.XyPadForeground;
    if(XyPadBackground == null) XyPadBackground = def.XyPadBackground;
    if(ButtonOn == null) ButtonOn = def.ButtonOn;
    if(ButtonOff == null) ButtonOff = def.ButtonOff;
  }
  return true;
}
